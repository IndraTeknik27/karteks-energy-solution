    ...
    <div class="container mx-auto px-4 sm:px-6 relative">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
            <div class="max-w-xl">
                ...
                @if($section->title)
                    <h2 class="text-3xl md:text-4xl lg:text-5xl font-bold leading-tight mb-4">{{ $section->title }}</h2>
                @endif
                @if($section->subtitle)
                    <p class="text-lg text-brand-50 leading-relaxed mb-6">{{ $section->subtitle }}</p>
                @endif
                <div class="flex flex-wrap gap-3">
                    <a href="{{ $ctaUrl }}" class="inline-flex items-center px-6 py-3 bg-white text-brand-700 font-semibold rounded-full hover:bg-brand-50 transition shadow-lg">
                        {{ $ctaLabel }}
                        <svg class="w-4 h-4 ml-2" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="m9 5 7 7-7 7"/></svg>
                    </a>
                    <a href="{{ route('services.show', 'custom-battery-service') }}" class="inline-flex items-center px-6 py-3 bg-white/10 backdrop-blur-sm border border-white/30 text-white font-semibold rounded-full hover:bg-white/20 transition">
