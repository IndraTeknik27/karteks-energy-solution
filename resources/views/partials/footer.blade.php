    ...
    <div class="container mx-auto px-4 sm:px-6 py-12">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            ...
            <div>
                ...
                <p class="text-sm leading-relaxed text-gray-400 mb-4">{{ config('karteks.company.tagline') }}</p>
                <div class="flex gap-3">
                    @if (config('karteks.company.whatsapp'))
                        <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $companyWhatsApp) }}" target="_blank" class="w-9 h-9 flex items-center justify-center rounded-full bg-gray-800 hover:bg-brand-600 transition" aria-label="WhatsApp">
                    ...
                    @endif
                    @if (config('karteks.company.email'))
                        <a href="mailto:{{ $companyEmail }}" class="w-9 h-9 flex items-center justify-center rounded-full bg-gray-800 hover:bg-brand-600 transition" aria-label="Email">
                    ...
                    @endif
                    @if (config('karteks.company.website'))
                        <a href="{{ config('karteks.company.website') }}" target="_blank" class="w-9 h-9 flex items-center justify-center rounded-full bg-gray-800 hover:bg-brand-600 transition" aria-label="Website">
            ...
            <div>
                ...
                <ul class="space-y-2 text-sm">
                    <li><a href="{{ route('catalog.index') }}" class="hover:text-brand-400 transition">Semua Produk</a></li>
                    <li><a href="{{ route('services.index') }}" class="hover:text-brand-400 transition">Layanan</a></li>
                    <li><a href="{{ route('blog.index') }}" class="hover:text-brand-400 transition">Blog &amp; Tips</a></li>
                    <li><a href="{{ route('cart.index') }}" class="hover:text-brand-400 transition">Keranjang</a></li>
                    @auth
                        <li><a href="{{ route('dashboard.index') }}" class="hover:text-brand-400 transition">Akun Saya</a></li>
                    @else
                        <li><a href="{{ route('login') }}" class="hover:text-brand-400 transition">Masuk</a></li>
                        <li><a href="{{ route('register') }}" class="hover:text-brand-400 transition">Daftar</a></li>
                    @endauth
                </ul>
            </div>
            ...
            <div>
                ...
                <ul class="space-y-2 text-sm">
                    @forelse ($footerPages as $page)
                        <li><a href="{{ route('pages.show', $page->slug) }}" class="hover:text-brand-400 transition">{{ $page->title }}</a></li>
                    @empty
                ...
                <ul class="space-y-3 text-sm">
                    @if ($companyPhone)
                        <li class="flex gap-2">
                            ...
                            <a href="tel:{{ $companyPhone }}" class="hover:text-brand-400 transition">{{ $companyPhone }}</a>
                        </li>
                    @endif
                    @if ($companyEmail)
                        <li class="flex gap-2">
                            ...
                            <a href="mailto:{{ $companyEmail }}" class="hover:text-brand-400 transition break-all">{{ $companyEmail }}</a>
                        </li>
                    @endif
                    @if ($companyAddress)
                        <li class="flex gap-2">
        ...
        <div class="border-t border-gray-800 mt-10 pt-6 text-xs text-gray-500 flex flex-col sm:flex-row gap-2 items-center justify-between">
            <p>&copy; {{ date('Y') }} {{ config('karteks.company.name') }}. All rights reserved.</p>
