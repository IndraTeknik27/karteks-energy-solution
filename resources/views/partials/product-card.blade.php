...
<a href="{{ $productUrl }}" class="group block bg-white rounded-2xl overflow-hidden border border-gray-100 hover:shadow-lg hover:border-brand-200 transition">
    <div class="aspect-square bg-gray-50 overflow-hidden relative">
        @php $cardImage = $product->featuredImageUrl; @endphp
        @if($cardImage)
            <img src="{{ $cardImage }}" alt="{{ $product->name }}" class="w-full h-full object-cover group-hover:scale-105 transition duration-500" loading="lazy">
        @else
    ...
    <div class="p-4">
        ...
        <div class="flex items-baseline gap-2 mt-2">
            <span class="text-base font-bold text-brand-700">{{ $priceFormatted }}</span>
            @if($oldPriceFormatted)
                <span class="text-xs text-gray-400 line-through">{{ $oldPriceFormatted }}</span>
            @endif
        </div>

        @if($product->manage_stock)
            <div class="text-[10px] text-gray-500 mt-1">
                Stok: {{ max(0, (int) $product->stock_qty - (int) ($product->reserved_qty ?? 0)) }}
            </div>
        @endif
    </div>
</a>