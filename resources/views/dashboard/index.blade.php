            ...
            <div class="container mx-auto px-4 sm:px-6 grid grid-cols-1 lg:grid-cols-4 gap-6">
                ...
                <aside>
                    <div class="bg-white rounded-2xl border border-gray-100 overflow-hidden">
                        ...
                        <nav class="text-sm">
                            <a href="{{ route('dashboard.index') }}" class="flex items-center gap-3 px-5 py-3 text-brand-700 bg-brand-50 font-semibold border-r-2 border-brand-600">
                            ...
                            <a href="{{ route('dashboard.orders') }}" class="flex items-center gap-3 px-5 py-3 text-gray-700 hover:bg-gray-50">
                            ...
                            <a href="{{ route('dashboard.wishlist') }}" class="flex items-center gap-3 px-5 py-3 text-gray-700 hover:bg-gray-50">
                            ...
                            <a href="{{ route('dashboard.review.my') }}" class="flex items-center gap-3 px-5 py-3 text-gray-700 hover:bg-gray-50">
                            ...
                            <a href="{{ route('dashboard.custom-battery.index') }}" class="flex items-center gap-3 px-5 py-3 text-gray-700 hover:bg-gray-50">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>
                                Custom Battery
                            </a>
                            <a href="{{ route('dashboard.quotation.index') }}" class="flex items-center gap-3 px-5 py-3 text-gray-700 hover:bg-gray-50">
                            ...
                            <a href="{{ route('dashboard.booking.index') }}" class="flex items-center gap-3 px-5 py-3 text-gray-700 hover:bg-gray-50">
                            ...
                            <a href="{{ route('dashboard.addresses') }}" class="flex items-center gap-3 px-5 py-3 text-gray-700 hover:bg-gray-50">
                            ...
                            <a href="{{ route('dashboard.profile.edit') }}" class="flex items-center gap-3 px-5 py-3 text-gray-700 hover:bg-gray-50">
                ...
                <div class="lg:col-span-3 space-y-6">
                    @if(session('success'))
                        <div class="p-3 bg-brand-50 border border-brand-200 text-brand-700 rounded-lg text-sm">{{ session('success') }}</div>
                    @endif
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                        ...
                        <div class="bg-white rounded-2xl border border-gray-100 p-5">
                            ...
                            <div class="text-2xl font-bold text-gray-900">{{ $addressCount }}</div>
                        ...
                        <div class="bg-white rounded-2xl border border-gray-100 p-5">
                            ...
                            <div class="text-2xl font-bold text-gray-900">{{ $user->hasVerifiedEmail() ? '✓' : '!' }}</div>
                            <div class="text-xs text-gray-500">Email {{ $user->hasVerifiedEmail() ? 'Terverifikasi' : 'Belum Verifikasi' }}</div>
                        </div>
                    </div>
                    ...
                    @if($recentOrders->isNotEmpty())
                        <div class="bg-white rounded-2xl border border-gray-100 overflow-hidden">
                            <div class="p-5 border-b border-gray-100 flex items-center justify-between">
                                ...
                                <a href="{{ route('dashboard.orders') }}" class="text-sm text-brand-600 hover:text-brand-700 font-medium">Lihat semua →</a>
                            </div>
                            <div class="divide-y divide-gray-100">
                                @foreach($recentOrders as $order)
                                    <a href="{{ route('dashboard.orders.show', $order->order_number) }}" class="flex items-center justify-between p-4 hover:bg-gray-50 transition">
                                        <div>
                                            <div class="font-semibold text-gray-900">{{ $order->order_number }}</div>
                                            <div class="text-xs text-gray-500">{{ $order->created_at?->format('d M Y H:i') }} • {{ $order->items->count() }} item</div>
                                        </div>
                                        <div class="text-right">
                                            <div class="font-semibold text-gray-900">Rp {{ number_format($order->total, 0, ',', '.') }}</div>
                                            <span class="text-xs px-2 py-0.5 rounded-full {{ $order->is_cancelled ? 'bg-red-100 text-red-700' : ($order->is_completed ? 'bg-brand-100 text-brand-700' : 'bg-amber-100 text-amber-700') }}">
                    ...
                    @else
                        <div class="bg-white rounded-2xl border border-gray-100 p-8 text-center">
                            <p class="text-gray-500">Belum ada pesanan. <a href="{{ route('catalog.index') }}" class="text-brand-600 font-medium">Mulai belanja</a></p>
                        </div>
                    @endif
                </div>
            </div>
