...
@section('content')
    {{-- Static Hero (always shown, di atas sections) --}}
    <section class="relative bg-gradient-to-br from-brand-700 via-brand-600 to-brand-800 text-white overflow-hidden">
        ...
        <div class="container mx-auto px-4 sm:px-6 py-16 md:py-24 lg:py-32 relative">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                ...
                @if(isset($heroFeaturedProducts) && $heroFeaturedProducts && $heroFeaturedProducts->isNotEmpty())
                    <div class="hidden lg:block">
                        <div class="grid grid-cols-2 gap-4">
                            @foreach($heroFeaturedProducts as $product)
                                <a href="{{ route('catalog.show', $product->slug) }}" class="group bg-white/10 backdrop-blur-sm border border-white/20 rounded-2xl p-4 hover:bg-white/20 transition">
                                    <div class="aspect-square rounded-xl bg-white/20 mb-3 overflow-hidden flex items-center justify-center">
                                        @if($product->getFirstMediaUrl('featured'))
                                            <img src="{{ $product->getFirstMediaUrl('featured') }}" alt="{{ $product->name }}" class="w-full h-full object-cover">
                                        @else
                                    ...
                                    <div class="text-xs text-brand-100 mt-1">{{ 'Rp '.number_format((float) ($product->sale_price ?? $product->price), 0, ',', '.') }}</div>
                                </a>
                            @endforeach
                        </div>
                    </div>
    ...
    {{-- Dynamic Sections dari HomepageService --}}
    @if(count($sections) > 0)
        @foreach($sections as $sectionData)
            ...
            @endphp
            @includeWhen(View::exists($view), $view, ['section' => $section, 'data' => $sectionDataArr])
        @endforeach
    @else
        {{-- Fallback: jika tidak ada section di DB (atau semua inactive) --}}
        @include('pages.home._fallback', ['faqs' => $faqs])
    @endif

    {{-- FAQ Section (selalu tampil sebagai fallback konten statis) --}}
    @if(count($sections) > 0 && $faqs->isNotEmpty())
    ...
    @if($popupBanners->isNotEmpty())
        ...
        <div x-data="{ open: false }" x-init="const seen = sessionStorage.getItem('popup_{{ $popup->id }}_seen'); if (!seen) { setTimeout(() => open = true, 1500); sessionStorage.setItem('popup_{{ $popup->id }}_seen', '1'); }" x-show="open" x-transition.opacity class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" style="display: none;">
            <div @click.outside="open = false" class="relative bg-white rounded-3xl overflow-hidden max-w-lg w-full shadow-2xl">
                ...
                @if($popup->desktop_image_url)
                    <img src="{{ $popup->desktop_image_url }}" alt="{{ $popup->title }}" class="w-full aspect-[3/4] object-cover">
                @endif
                <div class="p-6">
                    @if($popup->title)
                    ...
                    @if($popup->description)
                        <p class="text-sm text-gray-600 mb-4">{{ $popup->description }}</p>
                    @endif
                    @if($popup->link_url)
                        <a href="{{ $popup->link_url }}" target="{{ $popup->link_target ?? '_self' }}" class="inline-flex items-center px-5 py-2.5 bg-brand-600 text-white font-semibold rounded-full hover:bg-brand-700 transition">
                            {{ $popup->link_text ?: 'Lihat' }}
                            <svg class="w-4 h-4 ml-2" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="m9 5 7 7-7 7"/></svg>
                        </a>
                    @endif
                </div>
...
@push('scripts')
    <script>
        ...
        function trackBannerClick(bannerId) {
            const url = '{{ url("banners") }}/' + bannerId + '/click';
